...
const computeItems = (itemsInput) => {
  return itemsInput.map((item) => {
    ...
    const notes =
      ...
      normalizeNotes(item.specialInstructions) ||
      normalizeNotes(item.instructions) ||
      normalizeNotes(item.customerNote) ||
      normalizeNotes(item.remark) ||
      normalizeNotes(item.remarks) ||
      null;
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      await tx.voidLog.create({
        data: {
          ...
          transactionId: order.transactionId || order.orderCode,
          orderId,
          voidType: input.type,
